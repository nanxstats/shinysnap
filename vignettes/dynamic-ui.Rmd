...
Inputs created by `renderUI()` may not exist when a restore starts.
This vignette explains how shinysnap waits for them and how to read the
report that each restore produces.
...
there is no such input, Shiny drops the message without logging anything.

This causes problems when you restore inputs created by `renderUI()`.
Any message sent before the input is ready is lost. You can try adding
delays, but a delay that works on one machine may be too short on another.
Nested dynamic UI makes this harder because each set of inputs must wait
for the previous set.

## How shinysnap restores inputs

The server and shinysnap's browser script work together during a restore:
...
1. The server writes the tracked values back into your `reactiveValues`,
   ...
   browser in one message. Each value is converted to the format its input
   binding expects.
...
4. During the restore, the server makes the saved values available to
   Shiny's `restoreInput()` function. Shiny's input constructors call this
   function, so new inputs can start with their saved values already in
   the HTML. This avoids briefly showing defaults and triggering
   observers with those defaults. The browser reports these inputs as
   `constructed` without applying their values again.
...
The order of events matters here. Shiny sends a new input's initial value
to the server just after the `shiny:bound` event. Applying a saved value
inside that event handler would let the initial value overwrite it.
shinysnap waits until that step has finished before applying the saved value.
...
| `reapplied` | the input was recreated during the restore and received the value again, without help from `restoreInput()` |
...
timeout. `missing` and `failed` inputs produce a single warning
...
app, or that the saved state does not display its UI. Two common cases are:
...
The restore continues after `snap_restore()` returns. Its return value is
a *handle*: a list containing the restore id and a promise for the report.
You can receive the report through a callback, a hook, or the promise:
...
`promises::then()` inside an observer, end the observer with `NULL` as
above so it does not return a promise.

## Reacting during a restore

`snap_is_restoring()` is `TRUE` from the moment `snap_restore()` is called
until the report arrives. Use it to skip expensive calculations while
inputs are still changing, then run them when the restore finishes:
...
- Some inputs wait briefly before sending changes to the server. Text
  ...
  the server, which in practice gives these updates time to arrive.
  You don't need to add delays.
- Calling `snap_restore()` during a restore cancels the first restore, whose
...
- Use `snap_restore(use_restore_context = FALSE)` when debugging to
  disable the use of `restoreInput()`. Dynamic inputs still receive their
  saved values, but are reported as `applied` or `reapplied` instead of
  `constructed`. Their observers see the default value before the saved one.
